...
class(train_data)
zip.train[,1]
#comparing with zip.train

knn.fit <- train(y ~ ., method = "knn", data = data.frame("x" = train_data[,-1], "y" = as.factor(train_data[,1])),
      tune.grid = data.frame(k = seq(1,10,1)), trControl = control,
      allowParallel = TRUE) #Allows rstudio.cloud usage

knn.fit$bestTune
#Doesnt use our tunings, trying to use the 2 level factor as outcome column but creates errors too! Very frustrating.

levels((train_data[,1]))

factor(train_data[,1], labels = c("a","b"))

```

```{r}
library(glmnet)
train_lasso_fit <- cv.glmnet(train_data[,-1], train_data[,1], nfolds = 10, alpha = 1, type.measure = "class") #Using split with lasso/ridge
best_lambda = train_lasso_fit$lambda.min #most nonzero
plot(train_lasso_fit)
predict(train_lasso_fit, test_data[,-1], s = best_lambda, type = "coefficients")
